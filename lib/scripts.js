...
app.get('/jobs', async (req, res) => {
  ...
  if (!queueName) {
    return res.status(400).json({ error: 'El parámetro "queueName" es obligatorio' });
  }

  const myQueue = new Queque(queueName as string, bullConfig);

  // Mapa de funciones por estado
  const statusFunctions: Record<string, () => Promise<any[]>> = {
    waiting: () => myQueue.getWaiting(),
    active: () => myQueue.getActive(),
    delayed: () => myQueue.getDelayed(),
    completed: () => myQueue.getCompleted(),
    failed: () => myQueue.getFailed(),
  };

  try {
    if (!status) {
      // Si no se pasa status → devolvemos todo
    ...
    if (!statusFunctions[String(status)]) {
  return res.status(400).json({ error: `Estado inválido: "${status}"` });
}

    // Solo un estado específico
    const result = await statusFunctions[status as string]();
    return res.json({ [status as string]: result });

  } catch (error) {
    console.error('Error consultando la cola:', error);
